...
# We track the "develop" (main) branch instead of a pinned release so
# that you get the latest package recipes and bug fixes. The --depth
# flag keeps the clone shallow so it downloads quickly.
...
1. Since we track the `develop` branch, try pulling the latest commits with `git pull` inside your spack clone. The package recipe may
have been fixed upstream. Note that upgrading can invalidate previously installed packages, so you may need to reinstall them.
