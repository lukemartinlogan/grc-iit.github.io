...
This example depends on [Boost](https://www.boost.org/). The tutorial Dev
Container already provides it, so you can build right away. On your own machine
you can install it with spack (see
[Installing HPC Software](../06-installing-hpc-software/01-building-with-spack.md)):
```bash
spack install boost
spack load boost
```
