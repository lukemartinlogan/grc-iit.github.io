...
process. However, knowing how the compiler works is helpful.
...
To run the program, give the path to the executable. The leading `./` tells the
shell to look for the program in the current directory:
...
Every program returns an integer **exit code** when it finishes. Our `main` ends
with `return 0`, and by convention `0` means success — any non-zero value signals
some kind of failure. On Linux, the shell stores the exit code of the most recent
command in `$?`, so you can inspect it with:
